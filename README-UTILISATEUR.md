...
- Backend API : le projet utilise un serveur local (ex. `http://localhost/api/...`). Vérifiez `lib/services/*` pour les endpoints.
...
JSON :
```bash
curl -X POST http://localhost/api/add_mission.php -H "Content-Type: application/json" -d '{"interpreter_id":1,"reference_devis":"R001","montant_mission":100}'
```

Form-encodé :
```bash
curl -X POST http://localhost/api/add_mission.php -d "interpreter_id=1&reference_devis=R001&montant_mission=100"
```
