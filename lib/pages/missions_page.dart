...
class _MissionsPageState extends State<MissionsPage> {
  ...
  Future<void> _showMissionForm({Map<String, dynamic>? mission, required int interpreterId}) async {
    ...
    final tarifCtrl = TextEditingController(text: mission?['tarif_horaire']?.toString() ?? '');
    
    DateTime? debutDate = DateTime.tryParse(mission?['debutmission']?.toString() ?? '');
    DateTime? finDate = DateTime.tryParse(mission?['finmission']?.toString() ?? '');
    final initialDuree = debutDate != null && finDate != null ? finDate.difference(debutDate).inHours : 0;
    final dureeCtrl = TextEditingController(text: initialDuree > 0 ? initialDuree.toString() : '');
          ...
          return AlertDialog(
            ...
            content: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                InkWell(
                  onTap: () async {
                    ...
                    if (picked != null) {
                      setState(() {
                        debutDate = picked;
                        debutCtrl.text = DateFormat('yyyy-MM-dd').format(picked);
                  ...
                  child: InputDecorator(
                    ...
                    child: Text(
                      debutDate != null ? DateFormat('yyyy-MM-dd').format(debutDate!) : 'Sélectionner une date',
                      style: TextStyle(color: debutDate != null ? Colors.black : Colors.grey),
                    ),
                  ),
                ),
                const SizedBox(height: 8),
                InkWell(
                  onTap: () async {
                    ...
                    if (picked != null) {
                      setState(() {
                        finDate = picked;
                        finCtrl.text = DateFormat('yyyy-MM-dd').format(picked);
                        updateDureeFromDates();
                      });
                    }
                  },
                  child: InputDecorator(
                    ...
                    child: Text(
                      finDate != null ? DateFormat('yyyy-MM-dd').format(finDate!) : 'Sélectionner une date',
                      style: TextStyle(color: finDate != null ? Colors.black : Colors.grey),
                    ),
                  ),
                ),
                const SizedBox(height: 8),
                ...
                const SizedBox(height: 8),
                TextField(controller: tarifCtrl, decoration: const InputDecoration(labelText: 'Tarif horaire (€)'), keyboardType: TextInputType.number),
                CheckboxListTile(
                      ...
                      return _filtered.isEmpty
                          ...
                          : DsfrScrollbar(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) => _buildInterpreterExpandable(_filtered[i], canManage),
                              ),
                            );
                    },
                  ),
              ...
              child: Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    child: const Icon(Icons.arrow_drop_up, size: 16, color: Colors.black54),
                  ),
                ),
              ),
            ),
              ...
              child: Material(
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    height: 16,
                    alignment: Alignment.center,
                    child: const Icon(Icons.arrow_drop_down, size: 16, color: Colors.black54),
                  ),
                ),
              ),
            ),
  ...
  Widget _buildInterpreterExpandable(Map<String, dynamic> interp, bool canManage) {
    ...
    final phone = (interp['tel_mobile'] ?? interp['telMobile'] ?? '').toString();
    ...
    return Container(
      ...
      child: Theme(
        data: Theme.of(context).copyWith(
          ...
          splashColor: const Color(0xFF000091).withOpacity(0.05),
          highlightColor: const Color(0xFF000091).withOpacity(0.05),
        ),
        child: ExpansionTile(
            ...
            builder: (context, snapshot) {
              ...
              return Column(
                children: [
                    ...
                    Padding(
                      ...
                      child: SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          onPressed: () => _showMissionForm(interpreterId: id),
                  ...
                  ...missions.map((m) => _buildMissionExpandable(m, id, canManage)),
                ],
              );
            },
          ),
  ...
  Widget _buildMissionExpandable(Map<String, dynamic> m, int interpreterId, bool canManage) {
    final currency = NumberFormat.currency(locale: 'fr_FR', symbol: '€');
    final dateFmt = DateFormat('dd/MM/yyyy');
    
    final tarifHoraire = num.tryParse(m['tarif_horaire']?.toString() ?? '') ?? 0;
    ...
    final dureeHours = debut != null && fin != null ? fin.difference(debut).inHours : 0;
    final tarifText = tarifHoraire > 0 ? currency.format(tarifHoraire) : 'N/A';
    final montantTotal = tarifHoraire > 0 && dureeHours > 0 ? tarifHoraire * dureeHours : 0;
    final montantText = montantTotal > 0 ? currency.format(montantTotal) : 'N/A';
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          ...
          subtitle: Padding(
            ...
            child: Text(
              debut != null ? dateFmt.format(debut) : (m['debutmission'] ?? 'Date non définie'),
              style: const TextStyle(
                color: Color(0xFF666666),
                fontSize: 14,
              ),
          ...
          trailing: Row(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: paid ? const Color(0xFFB8FEC9) : const Color(0xFFFFE9E9),
                  borderRadius: BorderRadius.circular(4),
                  border: Border.all(
                    color: paid ? const Color(0xFF18753C) : const Color(0xFFCE0500),
                    width: 1,
                  ),
                ),
                child: Text(
          ...
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _buildInfoRow('Référence mission', m['reference_devis'] ?? 'Sans référence'),
              const SizedBox(height: 8),
              _buildInfoRow('Service', m['produit_ref'] ?? m['ref'] ?? 'N/A'),
              const SizedBox(height: 8),
              _buildInfoRow('Date de mission', debut != null ? dateFmt.format(debut) : (m['debutmission'] ?? 'N/A')),
              const SizedBox(height: 8),
              _buildInfoRow('Durée', dureeHours > 0 ? '${dureeHours}h' : 'N/A'),
              const SizedBox(height: 8),
              _buildInfoRow('Tarif horaire', tarifText),
              const SizedBox(height: 8),
              _buildInfoRow('Montant', montantText),
              if (canManage) ...[
                const Divider(
                  height: 32,
                  thickness: 1,
                  ...
                  children: [
                    OutlinedButton.icon(
                      ...
                      style: OutlinedButton.styleFrom(
                        foregroundColor: const Color(0xFF000091),
                        side: const BorderSide(color: Color(0xFF000091), width: 1),
                    ...
                    OutlinedButton.icon(
                      ...
                      style: OutlinedButton.styleFrom(
                        ...
                        side: const BorderSide(color: Color(0xFFCE0500), width: 1),
                        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(4),
                        ),
                      ...
                      onPressed: () async {
                        ...
                        try {
                          ...
                          if (ok) {
                            ...
                            setState(() => _detailFuture = MissionService.getMissionsByInterpreter(interpreterId));
                          } else {
