...
class _ExportPageState extends State<ExportPage> {
  ...
  Future<void> _exportInterpretersCsv() async {
    ...
    try {
      ...
      await _downloadText('interpretes.csv', csv, 'text/csv');
  ...
  Future<void> _exportUsersCsv() async {
    ...
    try {
      ...
      final rows = users
          ...
          .toList();
      final csv = _toCsv(rows);
      await _downloadText('utilisateurs.csv', csv, 'text/csv');
  ...
  Future<void> _exportMissionsTableCsv() async {
    ...
    final rows = _missions
        ...
        .toList();
    final csv = _toCsv(rows);
    await _downloadText('missions_table.csv', csv, 'text/csv');
  }
  ...
  Future<void> _downloadText(
    ...
    await downloadBytes(bytes: bytes, filename: filename, mimeType: mime);
  }
                      ...
                      : () async {
                          ...
                          try {
                            ...
                            final rows = all
                                ...
                                .toList();
                            final csv = _toCsv(rows);
                            await _downloadText('missions_all.csv', csv, 'text/csv');
                            _setBusy(false, msg: 'CSV exporté (${rows.length} lignes)');
                          } catch (e) {
                            _setBusy(false, msg: 'Erreur export CSV: $e');
                          }
                        },
              ...
              children: [
                ...
                DropdownButton<int>(
                  ...
                  items: const [10, 25, 50, 100, 200]
                      .map((v) => DropdownMenuItem(value: v, child: Text('Page: $v')))
                      .toList(),
                  onChanged: _busy
                      ? null
                      : (v) {
                          if (v != null) {
                            ...
                            _loadMissionsForTable(resetPage: true);
                          }
                        },
                ),
                const SizedBox(width: 12),
